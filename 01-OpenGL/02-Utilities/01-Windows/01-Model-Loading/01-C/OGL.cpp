    ...
    switch(iMsg)
    {
        ...
        case WM_CHAR:
            switch(wParam)
            {
                ...
                case 'L':
                case 'l':
                    bTogglePolygonMode = !bTogglePolygonMode;
                break;
...
int initialize(void)
{
    ...
    glBindVertexArray(vao);
    {
        ...
        glBufferData(
            ...
            gpVertexIndices->size * sizeof(GLint), 
            gpVertexIndices->ptr,
            GL_STATIC_DRAW
        );
...
void display(void)
{
    ...
    glUseProgram(shaderProgramObject);
    {
        ...
        rotationMatrix = vmath::rotate(modelAngle, 0.0f, 1.0f, 0.0f);
...
void uninitialize(void)
{
    ...
    destroyIntVector(gpNormalIndices);
    destroyIntVector(gpTextureIndices);
    destroyIntVector(gpVertexIndices);

    destroyFloatVector(gpNormal);
    destroyFloatVector(gpTexture);
    destroyFloatVector(gpVertex);
