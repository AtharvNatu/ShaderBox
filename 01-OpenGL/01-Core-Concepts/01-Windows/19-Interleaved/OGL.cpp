...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(
        WS_EX_APPWINDOW,
        szAppName,
        TEXT("OpenGL : Interleaved Rendering"),
...
int initialize(void)
{
    ...
    glBindVertexArray(vao_cube);
    {
        ...
        {   
            // 11 => Position(3) + Color(3) + Normal(3) + Texcoord(2)
            // sizeof(cube_pcnt) = 11 * 24 * sizeof(float) / 264 * sizeof(float)
            glBufferData(GL_ARRAY_BUFFER, sizeof(cube_pcnt), cube_pcnt, GL_STATIC_DRAW);
            
            //! Position
            glVertexAttribPointer(ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 11 * sizeof(GLfloat), (void*) 0);
            glEnableVertexAttribArray(ATTRIBUTE_POSITION);
            
            //! Color
            glVertexAttribPointer(ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 11 * sizeof(GLfloat), (void*) (3 * sizeof(GLfloat)));
            glEnableVertexAttribArray(ATTRIBUTE_COLOR);
            
            //! Normal
            glVertexAttribPointer(ATTRIBUTE_NORMAL, 3, GL_FLOAT, GL_FALSE, 11 * sizeof(GLfloat), (void*) (6 * sizeof(GLfloat)));
            glEnableVertexAttribArray(ATTRIBUTE_NORMAL);
            
            //! Texcoords
            glVertexAttribPointer(ATTRIBUTE_TEXTURE0, 2, GL_FLOAT, GL_FALSE, 11 * sizeof(GLfloat), (void*) (9 * sizeof(GLfloat)));
    ...
    // Load Texture
    if (loadGLTexture(&texture_marble, MAKEINTRESOURCE(IDBITMAP_CUBE)) == FALSE)
...
BOOL loadGLTexture(GLuint* texture, TCHAR imageResourceID[])
{
    ...
    //! Step Parameter Value For Packing/Unpacking (Set to 1 for better performance)
    ...
    if (hBitmap == NULL)
    {
        fprintf(gpFile, "ERROR : %s() => Failed to load bitmap !!!\n", __func__);
        return FALSE;
    }

    GetObject(hBitmap, sizeof(bmp), &bmp);
    
    glPixelStorei(GL_UNPACK_ALIGNMENT, stepValue);
