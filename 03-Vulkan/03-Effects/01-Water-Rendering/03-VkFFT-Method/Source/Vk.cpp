...
VkResult recordCommandBufferForImage(uint32_t imageIndex)
{
    ...
    VkFFTResult vkFFTResult = VkFFTAppend(&ocean->vkFFTApplication, -1, &vkFFTLaunchParams);
    if (vkFFTResult != VKFFT_SUCCESS)
        fprintf(gpFile, "VkFFTAppend() Failed : %d\n", (int)vkFFTResult);

    VkMemoryBarrier vkMemoryBarrier;
    memset((void*)&vkMemoryBarrier, 0 ,sizeof(VkMemoryBarrier));
    vkMemoryBarrier.sType = VK_STRUCTURE_TYPE_MEMORY_BARRIER;
    vkMemoryBarrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
    vkMemoryBarrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
    ...
    vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, ocean->vkPipeline_compute);
    vkCmdBindDescriptorSets(
        commandBuffer,
        VK_PIPELINE_BIND_POINT_COMPUTE,
        ocean->vkPipelineLayout_compute,
        0,
        1,
        &ocean->vkDescriptorSet_compute,
        0,
        NULL
    );
    ...
    uint32_t groupX = (ocean->pushData.tileSize + 15) / 16;
    uint32_t groupY = (ocean->pushData.tileSize + 15) / 16;
    vkCmdDispatch(commandBuffer, groupX, groupY, 1);

    //* Compute Write -> Graphics Vertex Input Read
    memset((void*)&vkMemoryBarrier, 0 ,sizeof(VkMemoryBarrier));
    vkMemoryBarrier.sType = VK_STRUCTURE_TYPE_MEMORY_BARRIER;
    vkMemoryBarrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
    vkMemoryBarrier.dstAccessMask = VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT;
