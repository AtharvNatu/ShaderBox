...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(
        WS_EX_APPWINDOW,
        szAppName,
        TEXT("Atharv Natu : cuFFT Vulkan-CUDA Interop"),
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
        ...
        switch (wParam)
        {
            ...
            case VK_LEFT:
                ocean->camera->rotateYaw(ocean->dt);
            break;
            case VK_RIGHT:
                ocean->camera->rotateYaw(-ocean->dt);
            break;
            ...
            case VK_UP:
                ocean->camera->roatatePitch(ocean->dt);
            break;
            case VK_DOWN:
                ocean->camera->roatatePitch(-ocean->dt);
            break;
    ...
    case WM_CHAR:
        ...
        switch (wParam)
        {
            case 'W':
            case 'w':
                ocean->camera->move(ocean->dt, FORWARD);
            break;
            case 'S':
            case 's':
                ocean->camera->move(ocean->dt, BACKWARD);
            break;
            case 'D':
            case 'd':
                ocean->camera->move(ocean->dt, RIGHT);
            break;
            case 'A':
            case 'a':
                ocean->camera->move(ocean->dt, LEFT);
            break;
...
VkResult resize(int width, int height)
{
    ...
    bInitialized = FALSE;
    {
        ...
        ocean->resize(winWidth, winHeight);
...
void uninitialize(void)
{
    ...
    if (uniformData_lighting.vkDeviceMemory)
    {
        ...
        fprintf(gpFile, "%s() => vkFreeMemory() Succeeded For uniformData_lighting.vkDeviceMemory\n", __func__);
    }
    ...
    if (uniformData_lighting.vkBuffer)
    {
        ...
        fprintf(gpFile, "%s() => vkDestroyBuffer() Succedded For uniformData_lighting.vkBuffer\n", __func__);
    }
    ...
    if (uniformData_oceanParams.vkDeviceMemory)
    {
        ...
        fprintf(gpFile, "%s() => vkFreeMemory() Succeeded For uniformData_oceanParams.vkDeviceMemory\n", __func__);
    }
    ...
    if (uniformData_oceanParams.vkBuffer)
    {
        ...
        fprintf(gpFile, "%s() => vkDestroyBuffer() Succedded For uniformData_oceanParams.vkBuffer\n", __func__);
    }
    ...
    if (uniformData_vbo.vkDeviceMemory)
    {
        ...
        fprintf(gpFile, "%s() => vkFreeMemory() Succeeded For uniformData_vbo.vkDeviceMemory\n", __func__);
    }
    ...
    if (uniformData_vbo.vkBuffer)
    {
        ...
        fprintf(gpFile, "%s() => vkDestroyBuffer() Succedded For uniformData_vbo.vkBuffer\n", __func__);
    }
...
VkResult buildCommandBuffers(void)
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        {
           ocean->buildCommandBuffers(vkCommandBuffer_array[i]);
        }
        //* Step - 7
