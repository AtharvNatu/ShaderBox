...
void Ocean::generate_fft_data(float time)
{
    ...
    for (int i = 0; i < N * M; ++i)
    {
        ...
        displacement_map[index] = glm::vec3(
            (n - N / 2.0f) * x_length / N - sign * lambda * out_displacement_x[index][0],
            sign * out_height[index][0],
            (m - M / 2.0f) * z_length / M - sign * lambda * out_displacement_z[index][0]
        );
    }
