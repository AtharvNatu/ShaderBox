...
glm::vec2 omega(1, 1);      // Wind Direction
float fTime = 0.0f;
...
VkDeviceSize meshSize = sizeof(glm::vec3) * N * M;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(
        WS_EX_APPWINDOW,
        szAppName,
        TEXT("Atharv Natu : Vulkan Ocean Using FFTW"),
...
void buildTessendorfMesh(Ocean* _ocean)
{
    ...
    memcpy(mappedPtr, _ocean->displacement_map, meshSize);
    memcpy((char*)mappedPtr + meshSize, _ocean->normal_map, meshSize);
}
...
VkResult createVertexBuffer(void)
{
    ...
    vkBufferCreateInfo.size = meshSize * 2;
    ...
    //* Step - 11
    vkResult = vkMapMemory(vkDevice, vertexData.vkDeviceMemory, 0, vkMemoryAllocateInfo.allocationSize, 0, &mappedPtr);
    if (vkResult != VK_SUCCESS)
    ...
    vkBufferCreateInfo.flags = 0;   //! Valid Flags are used in sparse(scattered) buffers
    vkBufferCreateInfo.pNext = NULL;
    vkBufferCreateInfo.size = numElements * sizeof(unsigned int);
    ...
    //* Step - 12
    memcpy(data, indices, numElements * sizeof(unsigned int));
...
VkResult updateUniformBuffer(void)
{
    ...
    mvpData.viewMatrix = camera.getViewMatrix();
...
VkResult buildCommandBuffers(void)
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        vkCmdBeginRenderPass(vkCommandBuffer_array[i], &vkRenderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
        {
            ...
            //! Bind with Vertex Buffer (Interleaved)
            VkDeviceSize vkDeviceSize_offset_array[2] = { 0, meshSize };
            VkBuffer vertexBuffers[] = { vertexData.vkBuffer, vertexData.vkBuffer };
