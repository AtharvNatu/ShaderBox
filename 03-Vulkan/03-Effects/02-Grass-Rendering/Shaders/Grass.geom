...
void createQuad(vec3 basePosition, mat4 crossModel)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		gl_Position = ubo.projectionMatrix * ubo.viewMatrix * (gl_in[0].gl_Position + modelWindMatrix * modelRandomYRotation * crossModel * (vertices[i] * grassSize));
...
{
	// Variable Declarations
	vec3 cameraDistance = gl_in[0].gl_Position.xyz - ubo.cameraPosition.xyz;
