...
namespace Overlay
{
    ...
    )
    {
        ...
        io.Fonts->AddFontFromFileTTF("../Assets/Poppins-Regular.ttf", fontSize, NULL, io.Fonts->GetGlyphRangesDefault());

        ImGui::StyleColorsDark();
        ImGuiStyle& style = ImGui::GetStyle();
        style.Colors[ImGuiCol_ResizeGrip]        = ImVec4(1.0f, 1.0f, 1.0f, 0.40f);
        style.Colors[ImGuiCol_ResizeGripHovered] = ImVec4(1.0f, 1.0f, 1.0f, 0.70f);
        style.Colors[ImGuiCol_ResizeGripActive]  = ImVec4(1.0f, 1.0f, 1.0f, 0.90f);
            
        vkResult = Overlay::createFontTexture();
        if (vkResult != VK_SUCCESS)
            fprintf(logFile, "%s() => createFontTexture() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
        else 
            fprintf(logFile, "%s() => createFontTexture() Succeeded\n", __func__);

        vkResult = Overlay::createShaders();
        if (vkResult != VK_SUCCESS)
            fprintf(logFile, "%s() => createShaders() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
        else 
            fprintf(logFile, "%s() => createShaders() Succeeded\n", __func__);

        vkResult = Overlay::createDescriptorPool();
        if (vkResult != VK_SUCCESS)
            fprintf(logFile, "%s() => createDescriptorPool() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
        else 
            fprintf(logFile, "%s() => createDescriptorPool() Succeeded\n", __func__);

        vkResult = Overlay::createDescriptorSetLayout();
        if (vkResult != VK_SUCCESS)
            fprintf(logFile, "%s() => createDescriptorSetLayout() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
        else 
            fprintf(logFile, "%s() => createDescriptorSetLayout() Succeeded\n", __func__);

        vkResult = Overlay::createDescriptorSet();
        if (vkResult != VK_SUCCESS)
            fprintf(logFile, "%s() => createDescriptorSet() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
        else 
            fprintf(logFile, "%s() => createDescriptorSet() Succeeded\n", __func__);

        vkResult = Overlay::createPipelineLayout();
        if (vkResult != VK_SUCCESS)
            fprintf(logFile, "%s() => createPipelineLayout() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
        else 
            fprintf(logFile, "%s() => createPipelineLayout() Succeeded\n", __func__);

        vkResult = Overlay::createPipeline();
        if (vkResult != VK_SUCCESS)
            fprintf(logFile, "%s() => createPipeline() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
        else 
            fprintf(logFile, "%s() => createPipeline() Succeeded\n", __func__);
    ...
    void RegisterWin32MsgHandler(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        {
            case WM_MOUSEMOVE:
                addMouseMoveHandler(lParam);
            break;

            case WM_LBUTTONDOWN:
                SetCapture(hwnd);
                addMouseButtonHandler(0, true);
            break;

            case WM_LBUTTONUP:
                ReleaseCapture();
                addMouseButtonHandler(0, false);
            break;

            case WM_RBUTTONDOWN:
                SetCapture(hwnd);
                addMouseButtonHandler(1, true);
            break;

            case WM_RBUTTONUP:
                ReleaseCapture();
                addMouseButtonHandler(1, false);
            break;

            case WM_MBUTTONDOWN:
                SetCapture(hwnd);
                addMouseButtonHandler(2, true);
            break;

            case WM_MBUTTONUP:
                ReleaseCapture();
                addMouseButtonHandler(2, false);
            break;

            case WM_MOUSEHWHEEL:
                addMouseWheelHandler(wParam);
            break;
    ...
    namespace
    {
        ...
        VkResult createBuffer(BufferData* bufferData, VkBufferUsageFlagBits bufferUsageFlagBits, VkDeviceSize bufferSize)
        {
            ...
            if (vkResult != VK_SUCCESS)
                fprintf(logFile, "%s() => vkCreateBuffer() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
            ...
            if (vkResult != VK_SUCCESS)
                fprintf(logFile, "%s() => vkAllocateMemory() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
            ...
            if (vkResult != VK_SUCCESS)
                fprintf(logFile, "%s() => vkBindBufferMemory() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
        ...
        VkResult createFontTexture()
        {
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkCreateBuffer() Failed For Staging Buffer : %s !!!\n", __func__, getVkResultString(vkResult));
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkAllocateMemory() Failed For Staging Buffer : %s !!!\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            { 
                fprintf(logFile, "%s() => vkBindBufferMemory() Failed For Staging Buffer : %s !!!\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkMapMemory() Failed For Staging Buffer : %s !!!\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkCreateImage() Failed For Font Texture : %s !!!\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkAllocateMemory() Failed For Font Texture : %s !!!\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkBindImageMemory() Failed For Font Texture : %s !!!\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkAllocateCommandBuffers() Failed For vkCommandBuffer_transition_image_layout : %s\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkBeginCommandBuffer() Failed For vkCommandBuffer_transition_image_layout : %s\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            {
                fprintf(logFile, "ERROR : %s() => Unsupported Texture Layout Transition !!!\n", __func__);
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "ERROR : %s() => vkEndCommandBuffer() Failed For vkCommandBuffer_transition_image_layout : %s\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "ERROR : %s() => vkQueueSubmit() Failed For vkSubmitInfo_transition_image_layout : %s\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "ERROR : %s() => vkQueueWaitIdle() Failed For vkSubmitInfo_transition_image_layout : %s\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkAllocateCommandBuffers() Failed For vkCommandBuffer_buffer_to_image_copy : %s\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "ERROR : %s() => vkBeginCommandBuffer() Failed For vkCommandBuffer_buffer_to_image_copy : %s\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "ERROR : %s() => vkEndCommandBuffer() Failed For vkCommandBuffer_buffer_to_image_copy : %s\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "ERROR : %s() => vkQueueSubmit() Failed For vkSubmitInfo_buffer_to_copy : %s\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
                ...
                fprintf(logFile, "%s() => vkQueueSubmit() Succeeded For vkSubmitInfo_buffer_to_copy\n", __func__);

            vkResult = vkQueueWaitIdle(vkQueue);
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "ERROR : %s() => vkQueueWaitIdle() Failed For vkCommandBuffer_buffer_to_image_copy : %s\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkCreateImageView() Failed For Font Texture : %s !!!\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
                return vkResult;
            }   
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkCreateSampler() Failed For Font Texture : %s !!!\n", __func__, getVkResultString(vkResult));
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
                return vkResult;
            }    
        ...
        VkResult createShaders(void)
        {
            ...
            //! ---------------------------------------------------------------------------------------------------------------------------
            //* Step - 6
            const char* szFileName = "Overlay.vert.spv";
            FILE *fp = NULL;
            size_t size;
            ...
            if (fp == NULL)
            {
                fprintf(logFile, "%s() => Failed To Open SPIR-V Shader File : %s !!!", __func__, szFileName);
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
                return vkResult;
            }
            else
                fprintf(logFile, "%s() => Succeeded In Opening SPIR-V Shader File : %s\n", __func__, szFileName);

            fseek(fp, 0L, SEEK_END);
            size = ftell(fp);
            if (size == 0)
            {
                fprintf(logFile, "%s() => Empty SPIR-V Shader File : %s !!!", __func__, szFileName);
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
                return vkResult;
            }
            ...
            if (shaderData == NULL)
            {
                fprintf(logFile, "%s() => malloc() Failed For shaderData !!!\n", __func__);
            ...
            if (retVal != 1)
            {
                fprintf(logFile, "%s() => Failed To Read From SPIR-V Shader File : %s !!!", __func__, szFileName);
                vkResult = VK_ERROR_INITIALIZATION_FAILED;
                return vkResult;
            }
            else
                fprintf(logFile, "%s() => Successfully Read Shader From SPIR-V Shader File : %s\n", __func__, szFileName);
            
            if (fp)
            {
                fclose(fp);
                fp = NULL;
                fprintf(logFile, "%s() => Closed SPIR-V File : %s\n", __func__, szFileName);
            }
            ...
            if (vkResult != VK_SUCCESS)
                fprintf(logFile, "%s() => vkCreateShaderModule() Failed For Vertex Shader : %s !!!\n", __func__, getVkResultString(vkResult));
            else
                fprintf(logFile, "%s() => vkCreateShaderModule() Succeeded For Vertex Shader\n", __func__);
            ...
            if (shaderData)
            {
                free(shaderData);
                shaderData = NULL;
                fprintf(logFile, "%s() => free() Succeeded For shaderData\n", __func__);
            }

            fprintf(logFile, "%s() => Vertex Shader Module Successfully Created\n", __func__);
            //! ---------------------------------------------------------------------------------------------------------------------------

            //! Fragment Shader
            //! ---------------------------------------------------------------------------------------------------------------------------
            szFileName = "Overlay.frag.spv";
            ...
            if (vkResult != VK_SUCCESS)
                fprintf(logFile, "%s() => vkCreateShaderModule() Failed For Fragment Shader : %s !!!\n", __func__, getVkResultString(vkResult));
            else
                fprintf(logFile, "%s() => vkCreateShaderModule() Succeeded For Fragment Shader\n", __func__);
            ...
            fprintf(logFile, "%s() => Fragment Shader Module Successfully Created\n", __func__);
            //! ---------------------------------------------------------------------------------------------------------------------------
        ...
        VkResult createDescriptorPool(void)
        {
            ...
            if (vkResult != VK_SUCCESS)
                fprintf(logFile, "%s() => vkCreateDescriptorPool() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
            else
                fprintf(logFile, "%s() => vkCreateDescriptorPool() Succeeded\n", __func__);
        ...
        VkResult createDescriptorSetLayout(void)
        {
            ...
            if (vkResult != VK_SUCCESS)
                fprintf(logFile, "%s() => vkCreateDescriptorSetLayout() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
            else
                fprintf(logFile, "%s() => vkCreateDescriptorSetLayout() Succeeded\n", __func__);
        ...
        VkResult createDescriptorSet(void)
        {
            ...
            if (vkResult != VK_SUCCESS)
            {
                fprintf(logFile, "%s() => vkAllocateDescriptorSets() Failed For vkDescriptorSet_imgui : %s !!!\n", __func__, getVkResultString(vkResult));
                return vkResult;
            }
            else
                fprintf(logFile, "%s() => vkAllocateDescriptorSets() Succeeded For vkDescriptorSet_imgui\n", __func__);
        ...
        VkResult createPipelineLayout(void)
        {
            ...
            if (vkResult != VK_SUCCESS)
                fprintf(logFile, "%s() => vkCreatePipelineLayout() Failed : %s !!!\n", __func__, getVkResultString(vkResult));
            else
                fprintf(logFile, "%s() => vkCreatePipelineLayout() Succeeded\n", __func__);
        ...
        VkResult createPipeline(void)
        {
            ...
            if (vkResult != VK_SUCCESS)
                fprintf(logFile, "%s() => vkCreatePipelineCache() Failed : %d !!!\n", __func__, vkResult);
            else
                fprintf(logFile, "%s() => vkCreatePipelineCache() Succeeded\n", __func__);
            ...
            if (vkResult != VK_SUCCESS)
                fprintf(logFile, "%s() => vkCreateGraphicsPipelines() Failed : %d !!!\n", __func__, vkResult);
            else
                fprintf(logFile, "%s() => vkCreateGraphicsPipelines() Succeeded\n", __func__);
            ...
            if (vkPipelineCache)
            {
                ...
                fprintf(logFile, "%s() => vkDestroyPipelineCache() Succeeded\n", __func__);
            }
        ...
        void updateBuffers(uint32_t imageIndex)
        {
            ...
            {
                Overlay::unmapBufferMemory(&vertexBuffers[imageIndex]);
                
                Overlay::destroyBuffer(&vertexBuffers[imageIndex]);

                vkResult = Overlay::createBuffer(&vertexBuffers[imageIndex], VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, vertexBufferSize);
                if (vkResult != VK_SUCCESS)
                    fprintf(logFile, "%s() => createBuffer() Failed For Vertex Buffer: %s !!!", __func__, getVkResultString(vkResult));

                vertexCounts[imageIndex] = imDrawData->TotalVtxCount;

                vkResult = Overlay::mapBufferMemory(&vertexBuffers[imageIndex]);
                if (vkResult != VK_SUCCESS)
                    fprintf(logFile, "%s() => mapBufferMemory() Failed For Vertex Buffer: %s !!!", __func__, getVkResultString(vkResult));
            ...
            {
                Overlay::unmapBufferMemory(&indexBuffers[imageIndex]);
                
                Overlay::destroyBuffer(&indexBuffers[imageIndex]);

                vkResult = Overlay::createBuffer(&indexBuffers[imageIndex], VK_BUFFER_USAGE_INDEX_BUFFER_BIT, indexBufferSize);
                if (vkResult != VK_SUCCESS)
                    fprintf(logFile, "%s() => createBuffer() Failed For Index Buffer : %s !!!", __func__, getVkResultString(vkResult));

                indexCounts[imageIndex] = imDrawData->TotalIdxCount;

                vkResult = Overlay::mapBufferMemory(&indexBuffers[imageIndex]);
                if (vkResult != VK_SUCCESS)
                    fprintf(logFile, "%s() => mapBufferMemory() Failed For Index Buffer: %s !!!", __func__, getVkResultString(vkResult));
            }
